  ...
  await User.find({ username: username, password: password })
    .then(result => {
      if (result.length === 0) {
        ...
        // 存在该用户切密码正确,发送token;
        const token = jwt.sign({ id: username }, app.get("secret"));
...
app.get("/getMessage/:username", async (req, res) => {
  ...
  // 获取用户信息
  // 获取未完成事务表
  const note = await Note.find({ username: username })
    .then(result => {
      return result;
    });
  await User.findOne({ username: username })
...
app.get("/addTask/:username/:noteContent", async (req, res) => {
  ...
  // 得到新ID
  const { noteID, noteNum, currentNoteNum } = await User.findOne({ username: username })
    .then(result => {
